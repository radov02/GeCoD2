...
def train(args):
    ...
    # depth-map cache: rank 0 generates missing maps, the datasets load them as the
    # image's 4th channel so the in-model depth ViT is skipped
    ...
    if depthmaps_dir_eff:
        ...
        # _pdf<k>: cached PCA-k decoder features, appended after the 4th-channel map
        if getattr(args, 'depthfeats_dir', ''):
            ...
            prepare_depthfeats(
                ...
                fit_ids=list(train_dataset.id_to_imgpath()),  # fit PCA on train ids
...
if __name__ == '__main__':
    ...
    parser.set_defaults(
        ...
        focal_alpha=0.25,
        aux_weight=0.3,
        # depth fusion, mode 1 for this experiment
        use_depth=1,
        depth_kernel_size=1, # 1x1 fusion, shell config default
        depth_feat_channels=16, # PCA-16 cache
    )
