...
def train(args):
    ...
    # per-run seed, offset by rank so ranks get different data/aug RNG
    ...
    # Hiera runs under no_grad in the forward, hence find_unused_parameters
    ...
    for n, p in model.named_parameters():
        if 'backbone' in n:
            # only backbone params unfrozen by --unfreeze_last_hiera
            if p.requires_grad:
                backbone_params[n] = p
        elif is_depth_fusion_param(n):
            # own LR group (--depth_fuse_lr, 0 = follow --lr)
            depth_fuse_params[n] = p
        elif args.use_density > 0 and ('density_head' in n or 'density_guide' in n):
            # own LR group (--density_lr)
            density_params[n] = p
        elif 'zs_prototypes' in n:
            # own LR group (--zs_proto_lr, 0 = follow --lr)
            zs_proto_params[n] = p
        else:
    ...
    if zs_proto_params:
        # not in aux_group_indices, the probe LR cut assumes [0] is the depth-fusion group
    ...
    if args.init_from_pretrained:
        # params missing from the checkpoint (depth-fusion convs, zs tokens) keep their init
        ...
        # strip the DDP module. prefix so checkpoint keys match
    ...
    if args.use_depth > 0:
        # snapshot after loading so the deviation log measures training only
    ...
    val_dataset = IOCFish5kDataset(
        ...
        num_objects=args.num_objects,
        tiling_p=args.tiling_p,
        # training=False adds scaling_factor+padwh, val counting matches inference_whole
        training=False,
        **density_kwargs,
    )
    loader_kwargs = dict(
    ...
    val_loader = DataLoader(
        ...
        batch_size=8,
        drop_last=False,
        # val returns the eval 7-tuple, needs pad_collate_test
    ...
    recover_count = 0  # divergence recoveries used so far (see --spike_recover_max)
    # plateau stop after --plateau_patience epochs with no val_select or NAE improvement
    plateau_count = 0
    ...
    if (args.probe_epochs > 0 or args.probe_adaptive == 1) and not depth_fuse_params:
        ...
        in_probe = False
    # body + backbone frozen during the probe, zs_prototypes keep training
    ...
    _probe_ran = False  # --probe_only: stop instead of unfreezing once the probe ends
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        for batch_idx, (img, bboxes, density_map, img_name, gt_bboxes) in enumerate(train_loader):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                for idx in range(img.shape[0]):
                    ...
                    alpha = 0
                    # small-object check: mean GT box width/height < 25 px
                    ...
                    # count = detection box-count, selection/plateau/LR all key off it
        ...
        with torch.no_grad():
            ...
            for batch_idx, (img, bboxes, density_map, img_name, gt_bboxes, scaling_factor, padwh) in enumerate(val_loader):
                ...
                for idx in range(img.shape[0]):
                    ...
                    nms_bboxes.append(boxes)
                    num_objects_pred.append(len(boxes))
                    # bbox counters @ IoU=0.5: greedy match by descending box_v
                ...
                # NAE = |gt - pred| / gt, clamp(min=1) for the gt=0 case
                val_nae += (
        ...
        if rank == 0:
            ...
            # depth-fusion deviation: L2 from the post-load snapshot, ~0 = still the RGB model
            ...
            # zs-prototype movement from init, empty in few-shot mode
            ...
            # densg gate: gamma is zero-init, any growth means the branch is doing something
            ...
            # adapter health: eff_rank ~1 = channel collapse, dead = GELU-suppressed fraction
            ...
            # depth-cue viz every --vis_every epochs (0 = off), one fixed val sample
            ...
            if val_rmse_val < best_spike_rmse:
                ...
                if recover_count < args.spike_recover_max:
                    # warm-restart: all ranks reload the best ckpt and cut the LR
                    recover_count += 1
            ...
            # adaptive-probe end / LR-cut decisions on rank 0, broadcast below
        ...
        if probe_end_flag.item() == 1:
            in_probe = False  # body unfreezes next epoch (probe_active recomputed at loop top)
            if rank == 0:
    ...
    if rank == 0:
        # the cancel handler runs the same path on SIGTERM/SIGINT
...
if __name__ == '__main__':
    ...
    # --zero_shot comes from the shell wrapper, which also suffixes MODEL_NAME _few/_zero
    parser.set_defaults(
        ...
        lr=2e-5,
        backbone_lr=0.0,
        lr_drop=50, # unused, ReduceLROnPlateau instead
        ...
        max_grad_norm=0.1,
        tiling_p=0.5,
        # crop_p comes from the shell's whole|tiled selector (0 whole, 0.5 tiled)
        # Loss coefficients
        giou_loss_coef=2,
        cost_class=2,
        cost_bbox=1,
    ...
    # defaults from training/config_IOCfish.sh, CLI flags still win
    from utils.run_config import apply_run_config_defaults
    apply_run_config_defaults(parser, dataset='IOCfish')
