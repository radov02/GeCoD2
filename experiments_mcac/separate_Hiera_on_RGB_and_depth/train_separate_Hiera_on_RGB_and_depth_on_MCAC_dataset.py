# MCAC version of the separate_Hiera_on_RGB_and_depth IOCfish training script.
import argparse
import os
import random
...
from utils.data import MCACDataset
...
def train(args):
    ...
    for n, p in model.named_parameters():
        if 'backbone' in n:
            # skip stages frozen by --unfreeze_last_hiera
            if p.requires_grad:
                backbone_params[n] = p
    ...
    if rank == 0 and zs_proto_params:
        print(f"[setup] zero-shot prototype optimizer group: {len(zs_proto_params)} params "
              ...
              flush=True)
    if rank == 0 and getattr(args, 'unfreeze_last_hiera', 0) > 0:
        print(f"[setup] backbone UNFROZEN: last {args.unfreeze_last_hiera} Hiera stage(s) "
              f"-> {len(backbone_params)} trainable params @ backbone_lr={args.backbone_lr:.2e}"
              + ("  WARNING: backbone_lr=0 -> nothing will train!" if args.backbone_lr == 0 else ""),
              flush=True)
    ...
    start_epoch = 0
    best = 10000000000000
    # best-density tracking, not used for selection
    best_dens = float('inf')
    ...
    if rank == 0:
        ...
        with open(txt_path, 'w') as f:
            ...
            f.write(f"\n{'Epoch':>6s}  {'TrainLoss':>10s}  {'ValLoss':>8s}  {'TrainMainLoss':>14s}  {'ValMainLoss':>12s}  {'TrainMAE':>9s}  {'ValMAE':>7s}  {'ValRMSE':>8s}  {'ValNAE':>7s}  {'ValDensMAE':>10s}  {'ValIoU':>7s}  {'ValGIoU':>8s}  {'ValF1':>6s}  {'LR':>9s}  {'EpochTime':>10s}  Best\n")
            f.write('-' * 133 + '\n')
    ...
    # write the summary footer even on scancel/time-limit/Ctrl-C
    cancel_state = {
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        # density-count val AE for the best-density checkpoint
        val_dens_ae = torch.tensor(0.0).to(device)
        ...
        with torch.no_grad():
            ...
            for batch_idx, (img, bboxes, density_map, img_name, gt_bboxes, scaling_factor, padwh) in enumerate(val_loader):
                ...
                # density count via full-map sum, fine on MCAC (square frames)
                if args.use_density and 'pred_density' in outputs[0]:
                    dens_counts = torch.tensor(
                        [outputs[i]['pred_density'].detach().float().sum().item()
                         for i in range(img.shape[0])])
                    val_dens_ae += torch.abs(num_objects_gt - dens_counts).sum()
        ...
        # density val MAE is only logged
        val_dens_mae = (val_dens_ae.item() / len(val_dataset)) if args.use_density else float('nan')
        ...
        if rank == 0:
            ...
            # per-level depth-pyramid gates, 3 gammas [src, l1, l2] with --sep_hiera_per_level_gate
            pyramid_gamma = model.module.pyramid_gamma_stats() if args.use_depth > 0 else {}
            if pyramid_gamma:
                pg_msg = (
                    f"  ~~~ pyramid gamma @epoch {epoch}: "
                    + ", ".join(f"{k}={v:.4e}" for k, v in pyramid_gamma.items())
                    + "\n"
                )
                print(pg_msg.strip())
                with open(txt_path, 'a') as f:
            ...
            # best-density checkpoint, selection stays box MAE
            if args.use_density and val_dens_mae < best_dens:
                best_dens = val_dens_mae
                torch.save(
                    {'epoch': epoch, 'model': model.state_dict()},
                    os.path.join(args.model_path, f'{args.model_name}_bestdens_{args.epochs}.pth')
                )
                cancel_state['dens_best'] = {'epoch': epoch, 'val_dens_mae': best_dens}
                print(
                    f"[rank0] epoch {epoch} -- saved best-DENSITY checkpoint "
                    f"(val_dens_mae={val_dens_mae:.2f}, box val_mae={val_mae_val:.2f}) "
                    f"-> {args.model_name}_bestdens_{args.epochs}.pth",
                    flush=True,
                )
            ...
            print(
                ...
                f"Val dens MAE: {val_dens_mae:.3f}",
            ...
            run.log({
                ...
                **({f"pyramid_gamma/{k}": v for k, v in pyramid_gamma.items()} if pyramid_gamma else {}),
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('GECO2-MCAC', parents=[get_argparser()])

    # MCAC defaults, CLI flags still win
    # --zero_shot comes from the shell wrapper
    parser.set_defaults(
        model_name='GECO2_MCAC',
        model_name_resumed='GECO2_MCAC',
        dataset='MCAC',
        ...
        max_grad_norm=0.1,
        tiling_p=0.5,
        # crop knobs come from the shared parser, MCAC runs stay whole-scale
        # Loss coefficients
        giou_loss_coef=2,
        cost_class=2,
        cost_bbox=1,
        ...
        focal_alpha=0.25,
        aux_weight=0.3,
        # Depth fusion, mode 4 = separate Hiera on the depth stream, pyramids summed
        use_depth=4,
        depth_feat_channels=3, # Hiera takes 3 channels
    )

    # pull defaults from training/config_MCAC.sh
    from utils.run_config import apply_run_config_defaults
    apply_run_config_defaults(parser, dataset='MCAC', exclude=('depth_feat_channels',))
