...
def _box_count_at(v, pred_boxes, abs_thr, nms_iou, img_shape, padwh):
    ...
    center = (b[:, :2] + b[:, 2:]) / 2
    valid = (center[:, 0] * img_shape[-2] < maxw) & (center[:, 1] * img_shape[-1] < maxh)
    return int(valid.sum())


@torch.no_grad()
def whole_val_sweep(model, device, args):
    """Grid-search abs box_v threshold x NMS IoU on the matching val split."""
    val_split = {'test': 'val', 'test_coco': 'val_coco'}.get(args.test_split, 'val')
    log(f"[whole_sweep] building VAL dataset (split={val_split}) for the threshold sweep...")
    val_ds = FSC147DATASET(
    ...
    for abs_thr in WHOLE_SWEEP_ABS_THRS:
        for nms_iou in WHOLE_SWEEP_NMS_IOUS:
            ae = sum(abs(gt - (_box_count_at(v, b, abs_thr, nms_iou, ishape, pad) if v.numel() else 0))
                     for v, b, gt, pad, ishape in cand)
...
def evaluate(args):
    ...
    if getattr(args, 'whole_sweep', 0) and args.test_split in ('test', 'test_coco'):
    ...
    for batch_i, (img, bboxes, density_map, ids, gt_bboxes, scaling_factor, padwh) in enumerate(
        ...
        pred_boxes_batch = []
        pred_scores_batch = []
        pred_coco_scores_batch = []  # mask-IoU scores for the COCO dump
        ...
        for idx in range(batch_size):
            ...
            if len(outputs[idx]['pred_boxes'][-1]) == 0:
                ...
                pred_coco_scores_batch.append(torch.empty(0))
            else:
                v = outputs[idx]["box_v"]
                ...
                scores = v[mask][keep]
                # COCO-dump score = SAM mask-IoU, same as the original inference.py
                coco_scores = outputs[idx]["scores"][mask][keep]
        ...
        for idx in range(batch_size):
            ...
            # rank by the mask-IoU scores
            match_scores = pred_coco_scores_batch[idx]
            ...
            # COCO dump in original px (/sf), scored by mask-IoU
            if args.coco_json_out:
                name = test_dataset.image_names[ids[idx].item()]
                coco_records.append((test_dataset.img_name_to_ori_id[name],
                                     pred_boxes / scaling_factor[idx].item(),
                                     pred_coco_scores_batch[idx]))
        ...
        for idx in range(batch_size):
            ...
            # image_names carry the extension, image_ids is the bare stem
            ...
            pairs = (
                match_pairs(pred_px, pred_coco_scores_batch[idx].detach().cpu(), gt_px, 0.5)
    ...
    _sre_full = primary_rows[0][1]['sre']  # MCAC Eq. 6, clamp max(gt,1)
    print(f"Test MAE:  {mae:.3f}")
    print(f"Test MSE*: {rmse:.3f}   (RMSE)")
    print(f"Test NAE:  {nae:.3f}")
    print(f"Test SRE:  {_sre_full:.3f}")
    ...
    with open(txt_path, 'w') as f:
        ...
        f.write("Bbox matching/AP + COCO dump ranked by SAM mask-IoU scores (was box_v -- counting/threshold/NMS unchanged, so count metrics are "
                "comparable to older runs but AP/P/R/F1 are not).\n")
        ...
        f.write(f"  Test SRE:    {_sre_full:.3f}\n")
        if _times:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('GECO2-FSCD147-Inference', parents=[get_argparser()])
    parser.add_argument(
