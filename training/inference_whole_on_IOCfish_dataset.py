...
def evaluate(args):
    ...
    for batch_i, (img, bboxes, density_map, ids, gt_bboxes, scaling_factor, padwh) in enumerate(
        ...
        # skip samples with a non-finite gt count (NaN/Inf density)
        ...
        for idx in range(batch_size):
            ...
            # original image from disk, .jpg then .png fallback
            img_id = test_dataset.image_ids[ids[idx].item()]
            orig_img_path = test_dataset.img_dir / f'{img_id}.jpg'
            if not orig_img_path.exists():
                orig_img_path = test_dataset.img_dir / f'{img_id}.png'
