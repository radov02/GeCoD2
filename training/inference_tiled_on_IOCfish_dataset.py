"""Tiled inference for GECO2 on IOCfish5k.

Whole-image inference shrinks 1920x1080 to 1024 and under-detects the tiny
fish, so the model runs on overlapping square crops instead. Sliced objects
are handled by the overlap, the inner-edge drop and a global NMS in original
coords; fish bigger than a tile come from the optional whole-image pass.
Runs zero-shot or few-shot (per-tile exemplars from the GT boxes).
"""
...
from utils.data import IOCFish5kDataset, pad_collate_test, resize_and_pad
...
def parse_tag_and_mode(model_name):
    """Split a 'GECO2_IOCfish_<tag>_<mode>' model_name into (tag, mode)."""
...
def decode_outputs(output):
    ...
    match_v = output["scores"].reshape(-1).float()  # (N,) mask-IoU (== box_v for >800-box images)
    return pred_boxes[0], box_v[0], match_v
...
def collect_panel_candidates(model, device, orig_img, gt_boxes_orig,
    ...
    # whole-image density count, the alternative --density_count_source
    _whole_dens_count = None
    ...
    whole_cands = dict(boxes=torch.empty((0, 4)), scores=torch.empty(0),
                       match_scores=torch.empty(0))
    if not panels:
        return tile_cands, whole_cands, None, None, None
    ...
    for start in range(0, len(panels), args.tile_batch_size):
        ...
        for i, panel in enumerate(batch):
            boxes_norm, scores, match_scores = decode_outputs(outputs[i])
            ...
            elif _use_density and panel['is_whole']:
                # whole-image density integral over the non-padded region
                _dmap = outputs[i]['pred_density'].float()
                _Hd, _Wd = _dmap.shape[-2:]
                _pw, _ph = panel['padwh']
                _vw = max(1, int(round((image_size - float(_pw)) / image_size * _Wd)))
                _vh = max(1, int(round((image_size - float(_ph)) / image_size * _Hd)))
                _whole_dens_count = float(_dmap[..., :_vh, :_vw].sum())
            ...
            if panel['is_whole']:
                ...
                boxes_norm, scores, match_scores = boxes_norm[keep], scores[keep], match_scores[keep]
                if boxes_norm.shape[0] == 0:
                    continue
    ...
    return tile_cands, whole_cands, density_count, _whole_dens_count, dens_map


def _resolve_density_count(tiled, whole, source):
    """Pick the headline density count: the tiled stitch, the whole-image
    integral, or the max of the two."""
    if source == 'whole':
        return whole if whole is not None else tiled
    if source == 'max':
        vals = [v for v in (tiled, whole) if v is not None]
...
def postprocess_candidates(tile_cands, whole_cands, orig_w, orig_h, args,
    ...
    return (boxes.detach().cpu(), scores.detach().cpu(), match_scores.detach().cpu(),
...
def tiled_predict(model, device, orig_img, gt_boxes_orig,
    ...
    _, orig_h, orig_w = orig_img.shape
    tile_cands, whole_cands, dc_tiled, dc_whole, dens_map = collect_panel_candidates(
        model, device, orig_img, gt_boxes_orig,
        whole_img, whole_bboxes, whole_sf, whole_padwh, args, orig_depth=orig_depth,
    )
    density_count = _resolve_density_count(
        dc_tiled, dc_whole, getattr(args, 'density_count_source', 'tiled'))
...
def _load_orig_and_gt(test_dataset, img_id, args):
    """Original unpadded RGB tensor and, in few-shot mode, GT boxes in original px."""
    orig_path = test_dataset.img_dir / f'{img_id}.jpg'
    if not orig_path.exists():
        orig_path = test_dataset.img_dir / f'{img_id}.png'
    ...
    if dm:
        ...
        orig_depth = load_depth_map(dm, img_id, (orig_img.shape[1], orig_img.shape[2]))
        df = getattr(test_dataset, 'depthfeats_dir', '')
        if df:
            # decoder-feature channels stacked after the 1-ch map
            orig_depth = torch.cat(
                [orig_depth, load_depth_feats(df, img_id, (orig_img.shape[1], orig_img.shape[2]))], dim=0)

    gt_boxes_orig = None
    if not args.zero_shot:
        anns = IOCFish5kDataset._parse_xml(test_dataset.xml_dir / f'{img_id}.xml')
        xml_boxes = [a['bbox'] for a in anns if a['bbox'] is not None]
        if xml_boxes:
            gt_boxes_orig = torch.tensor(xml_boxes, dtype=torch.float32)
...
def run_sweep(args, model, device, test_dataset, test_loader):
    ...
    for batch_i, (img, bboxes, density_map, ids, gt_bboxes, scaling_factor, padwh) in enumerate(
        ...
        _, orig_img, gt_boxes_orig, orig_depth = _load_orig_and_gt(test_dataset, img_id, args)
        _, orig_h, orig_w = orig_img.shape
        tile_cands, whole_cands, dc_tiled, dc_whole, _dens_map_unused = collect_panel_candidates(
            model, device, orig_img, gt_boxes_orig,
            img[0], bboxes[0], scaling_factor[0].item(),
            (int(padwh[0][0]), int(padwh[0][1])), args, orig_depth=orig_depth,
        )
        density_count = _resolve_density_count(dc_tiled, dc_whole, args.density_count_source)
    ...
    with open(out_path, 'w') as f:
        f.write(f"Tiled-inference parameter sweep on {n_eval} VAL images "
                f"(hyperparameter tuning; test untouched)\n")
        ...
        if use_density:
            f.write(
                ...
                f"(source={args.density_count_source}):\n"
...
def evaluate(args):
    ...
    # tag outputs for a non-default density-count source
    if getattr(args, 'use_density', 0) and getattr(args, 'density_count_source', 'tiled') != 'tiled':
        args.results_suffix = f"{args.results_suffix}_dc{args.density_count_source}"
        log(f"[density_count_source={args.density_count_source}] tagging outputs: results_suffix='{args.results_suffix}'")

    # val + sweep would tune and report on the same split
    if args.test_split == 'val' and (args.sweep or args.sweep_then_test):
        log("[warn] --test_split val together with a sweep tunes AND reports on val "
            "(calibration leak). Prefer whole inference for the val column; tiled-val "
            "should use fixed thresholds (drop --sweep/--sweep_then_test).")
    ...
    if args.sweep or args.sweep_then_test:
        log("building VAL dataset for the sweep (hyperparameter tuning must not touch test)...")
    ...
    log(f"tiled inference ({mode}): tile_size={args.tile_size} overlap={args.tile_overlap} "
        ...
        + (f" density_count_source={args.density_count_source}" if args.use_density else ""))
    ...
    for batch_i, (img, bboxes, density_map, ids, gt_bboxes, scaling_factor, padwh) in enumerate(
        ...
        orig_path, orig_img, gt_boxes_orig, orig_depth = _load_orig_and_gt(test_dataset, img_id, args)
    ...
    if density_rows is not None:
        print(f"Density-integral count metrics (SECONDARY, count = density.sum(), "
              f"source={args.density_count_source}):")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('GECO2-IOCfish-Tiled-Inference', parents=[get_argparser()])
    parser.add_argument(
    ...
    parser.add_argument(
        '--density_count_source', type=str, default='tiled',
        choices=['tiled', 'whole', 'max'],
        help="density-integral source with --use_density: 'tiled', 'whole' or 'max'",
    )
    parser.add_argument(
        '--tile_size', type=int, default=512,
        help='Square tile side, in original-image pixels (smaller = more zoom, more tiles).',
    )
    parser.add_argument(
        '--tile_overlap', type=int, default=128,
        help='Overlap (px) between adjacent tiles; should exceed the largest fish.',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="evaluation split: 'test' (2000 images) or 'val' (500)",
    )
